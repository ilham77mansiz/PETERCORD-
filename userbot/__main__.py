...
try:
    ...
    PLUGIN_MESAJLAR = {}
    ORJ_PLUGIN_MESAJLAR = {"simbole": "`🎖️THE PETERCORD USERBOT🎖️.`", "afk": f"`{str(choice(AFKSTR))}`", "kickme": "`Aku keluar dadah hiks`😑", "pm": UNAPPROVED_MSG, "dızcı": str(choice(DIZCILIK_STR)), "ban": "{mention}`, meresahkan!`", "mute": "{mention}`, kasian !`", "approve": "{mention}`, mampus lu!`", "disapprove": "{mention}`, sini kalian temenin aku!`", "block": "{mention}`, kasian di block!`"}
